...
SELECT
  ...
  -- DATA INTEGRATION on the gender column. We will keep the gender from crm_cust_info if it is not 'n/a', otherwise we will take the gender from erp_cust_az12 if it is not null, otherwise we will set it to 'n/a'.
...
SELECT
  ...
  ON pn.cat_id = pc.id
  WHERE prd_end_dt IS NULL; -- Assuming we only want active products in the gold layer, Filter out all historical data.
