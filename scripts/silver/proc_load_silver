	...
	DECLARE @Start_time DATETIME, @End_time DATETIME, @Start_batch_time DATETIME, @End_batch_time DATETIME;
	  BEGIN TRY
		SET @Start_batch_time = GETDATE();
			...
			-- BRONZE crm_prd_info
			-- Update table to match up with transformations done.
			...
			-- Insert data into silver.crm_prd_info with transformations and cleaning
			SET @Start_time = GETDATE() 
			...
			-- INSERT CLEAN RECORD TO SILVER LAYER
			SET @Start_time = GETDATE()
			...
			INSERT INTO silver.crm_sales_details(
			  ...
			  SELECT
				...
				CASE WHEN sls_price IS NULL OR sls_price <=0
					...
					ELSE sls_price
				END AS sls_price
			 FROM bronze.crm_sales_details; -- Now Perform the quality check again.
			 SET @End_time = GETDATE();
			 PRINT '>> Load Duration: ' + CAST(DATEDIFF(second, @Start_time, @End_time) AS NVARCHAR) + ' seconds';
			 PRINT '>> -------------';
